...
from maham.physics.spectra import convert_limit_normalization_to_decade_width, rescale_bandwidth_factor_to_decade_width, rescale_differential_decade_width
...
def test_half_decade_to_one_decade():
    ...
    np.testing.assert_allclose(rescale_differential_decade_width(values, 0.5, 1.0), [1.0, 2.0])


def test_one_decade_to_half_decade():
    values = np.array([2.0, 4.0])
    np.testing.assert_allclose(rescale_differential_decade_width(values, 1.0, 0.5), [4.0, 8.0])


def test_bandwidth_factor_to_one_decade():
    values = np.array([2.0, 4.0])
    np.testing.assert_allclose(rescale_bandwidth_factor_to_decade_width(values, 4.0), values * 4.0 / np.log(10.0))
...
def test_decade_width_must_be_positive(native, target):
    with pytest.raises(ValueError, match="finite positive"):
        rescale_differential_decade_width(np.array([1.0]), native, target)
