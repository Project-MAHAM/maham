...
def main():
    ...
    ax.loglog(table["energy"].to_value(u.GeV), table["E2phi"].to_value(UNIT), label="IceCube-Gen2 Radio 10 yr", **validation_curve_style("sensitivity"))
    ax.set_xlabel("Neutrino energy [GeV]", fontweight="bold")
    ax.set_ylabel(r"$E^2\Phi$ [GeV cm$^{-2}$ s$^{-1}$ sr$^{-1}$]", fontweight="bold")
    ax.set_title("IceCube-Gen2 Radio Diffuse Neutrino Sensitivity (2021)", fontweight="bold")
