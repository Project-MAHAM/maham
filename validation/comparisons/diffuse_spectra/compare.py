...
def load_datasets():
    ...
    datasets = {
        ...
        "fermi": get_dataset("fermi_lat.igrb.2015"),
    ...
    tables = {
        ...
        "icecube_2022": datasets["icecube_2022"].load(
            quantity=quantity, flavor="all_flavor", flavor_assumption="equal"
        ),
...
def validate_comparison(tables):
    ...
    require(np.count_nonzero(tables["fermi"]["is_upper_limit"]) == 1, "Fermi-LAT IGRB contributes one upper limit")
    require(bool(tables["fermi"]["is_upper_limit"][-1]), "Fermi-LAT IGRB upper limit is the final point")
...
def plot_fermi(ax, table, quantity, unit):
    ...
    handle = ax.errorbar(
        ...
        markersize=s["ms"], markeredgewidth=1.3, elinewidth=1.2, capsize=2.5, label=r"Diffuse $\gamma$ (Fermi-LAT)",
        zorder=10,
    )
    plot_upper_limits(
        ax, energy[upper], y[upper], xerr=xerr[:, upper], arrow_factor=2.5, color=s["color"],
...
def plot_icecube_piecewise(ax, table, quantity, unit, label):
    ...
    plot_upper_limits(
        ax, energy[upper], y_upper[upper], xerr=xerr[:, upper], arrow_factor=2.5, color=s["color"],
        linewidth=1.5, capsize=3, mutation_scale=12,
    )
    return handle
...
def plot_ehe_limit(ax, table, quantity, unit):
    ...
    y = table[quantity].to_value(unit)
    return ax.plot(
        energy, y, linestyle="-", linewidth=2.2, color=STYLE["ehe"]["color"],
        label="IceCube EHE UL (90% CL)", zorder=8,
    )[0]
...
def plot_comparison(tables):
    ...
    limit_sensitivity_legend = ax.legend(
        handles=limit_sensitivity_handles, loc="lower center", bbox_to_anchor=(0.5, 0.02), ncol=2, frameon=True
    )
    ...
    if model_handles:
        model_legend = ax.legend(
            handles=model_handles, loc="center left", bbox_to_anchor=(0.01, 0.5), ncol=1, frameon=True
        )
